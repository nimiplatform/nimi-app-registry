...
- After deterministic checks and human review, a current Registry maintainer
  may add one final commit to that same PR. The commit deletes the submission,
  adds one immutable descriptor below `descriptors/<app_id>/<version>.json`, and
  updates `index.json`.
